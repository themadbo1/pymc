...
if ((os.path.isfile(os.getcwd() + "/start.sh")) and (os.path.isfile(os.getcwd() + "\\server.jar"))):#checks if theres is an existing server
    print("Server.jar found in current directory.\nStarting server.")
    subprocess.run("sudo start.sh",shell=True)
    sys.exit(0)
...
def main():
    ...
    ram = ram*1024
    command = f"sudo java -Xmx{ram}M -Xms{ram}M -jar server.jar nogui" 
    with open("start.sh","w") as start_bat:
